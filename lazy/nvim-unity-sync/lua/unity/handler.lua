...
function XmlCsprojHandler:updateRoot()
    ...
    local root, err = utils.findRootFolder()
    if root then
        self.rootFolder = root
        -- print("[Debug] Found Unity project at: " .. root)  -- 调试信息
    else
        self.rootFolder = nil
        -- print("[Debug] No Unity project found: " .. (err or "unknown error"))  -- 调试信息
    end
end
...
function XmlCsprojHandler:validateProject()
    ...
    -- 检查关键目录和文件是否存在（更宽松的验证）
    local assetsExists = vim.fn.isdirectory(self.rootFolder .. "/Assets") == 1
    local csprojExists = utils.fileExists(filePath)
    
    if not assetsExists then
        return false, "No Assets folder found - not a Unity project"
    end
    
    if not csprojExists then
        return false, "No CsProject found, regenerate project files in Unity"
    end

    -- 尝试加载csproj文件
    local loaded, err = self:load(filePath)
    if not loaded then
        -- 即使不能加载，如果有Assets文件夹也认为是Unity项目
        if assetsExists then
            self.hasCSProjectUnityCapability = true
            return true, "Unity project detected (csproj load failed but Assets exists)"
        end
        return false, "Failed to load csproj: " .. (err or "unknown error")
    end

    -- 检查Unity capability，但即使没有也认为是Unity项目（只要有Assets）
    local hasUnityCapability = self:checkProjectCapability("Unity")
    
    if hasUnityCapability or assetsExists then
        self.hasCSProjectUnityCapability = true
        if hasUnityCapability then
            return true, "This is an Unity Project ready to sync"
        else
            return true, "Unity project detected (Assets folder exists)"
        end
    else
        return false, "Project file exists but doesn't have Unity capability and no Assets folder"
    end
end
...
function XmlCsprojHandler:checkProjectCapability(attribute)
    ...
    -- 多种匹配模式，适应不同的格式
    local patterns = {
        '<ProjectCapability.-Include="' .. attribute .. '".-/>',
        '<ProjectCapability.-Include=%s*"' .. attribute .. '"%s*/>',
        '<ProjectCapability%s[^>]*Include%s*=%s*"' .. attribute .. '"[^>]*>',
    }
